...
# Rutas de trabajo (Volviendo a .icons)
...
if [ "$OPT" == "2" ]; then
    ...
    rm -rf "$THEME_DIR" "$ICONS_DIR/${THEME_BASE}-Light" "$TEMPLATES_DIR/${THEME_NAME}.sh"
...
# --- INSTALACIÓN OFICIAL ---
info "Ejecutando instalador en $ICONS_DIR..."
bash "$CDIR/install.sh" -d "$ICONS_DIR" -n "$THEME_NAME" -t default -s default

# --- SNAPSHOT GIT (Obligatorio para que Noctalia cambie el color) ---
...
success "¡Instalación finalizada en .icons!"
info "Usa Noctalia para cambiar los colores."
