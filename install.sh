...
info "Descargando Papirus Icon Theme..."
...
success "Papirus descargado"

info "Creando $THEME_NAME..."
rm -rf "$THEME_DIR" && mkdir -p "$THEME_DIR"
for size in 16x16 22x22 24x24 32x32 48x48 64x64; do
    mkdir -p "$THEME_DIR/$size/places"
    find "$PAPIRUS_SRC/$size/places/" -name "*.svg" | while read -r f; do
        cp -L "$f" "$THEME_DIR/$size/places/" 2>/dev/null || true
    done
done
cp "$PAPIRUS_SRC/index.theme" "$THEME_DIR/index.theme"
sed -i "s/^Name=.*/Name=$THEME_NAME/" "$THEME_DIR/index.theme"
sed -i "s/^Inherits=.*/Inherits=Papirus-Dark,hicolor/" "$THEME_DIR/index.theme"
success "$THEME_NAME creado"
...
COLOR="${PRIMARY:1}"
COLOR2="${SECONDARY:1}"
THEME_DIR="$HOME/.icons/Noctalia-Folders"
...
if git -C "$THEME_DIR" show-ref --quiet "refs/heads/$BRANCH"; then
    ...
    find "$THEME_DIR" -name "*.svg" -print0 | xargs -0 sed -i \
...
if [[ "$CURRENT" == "Noctalia-Folders" ]]; then
    gsettings set org.gnome.desktop.interface icon-theme "hicolor" 2>/dev/null
    sleep 0.3
    gsettings set org.gnome.desktop.interface icon-theme "Noctalia-Folders" 2>/dev/null
else
    gsettings set org.gnome.desktop.interface icon-theme "Noctalia-Folders" 2>/dev/null
fi
